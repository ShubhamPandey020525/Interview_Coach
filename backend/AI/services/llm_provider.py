...
def _generate_local_question(
    ...
    # Your name is James. You are a senior technical interviewer conducting a job interview.
...
FILLER_WORD_SET = {"um", "uh", "er", "like", "you know", "so", "sort of", "kind of", "basically", "actually", "i mean"}
FILLER_WORDS = FILLER_WORD_SET
...
def _count_filler_words(text: str) -> int:
    ...
    while i < len(words):
        matched = False
        for filler in sorted(FILLER_WORD_SET, key=len, reverse=True):
            parts = filler.split()
            n = len(parts)
            if i + n <= len(words):
...
def _generate_personalized_best_answer_and_comparison(
    question: str,
    answer: str,
    target_role: str = "Software Engineer",
    resume_context: dict | None = None,
) -> tuple[str, str, str, dict]:
    """Generates dynamic, deeply technical best answers and personalized feedback.
    Never returns canned boilerplate strings. Quotes and evaluates the candidate's actual words.
    """
    ...
    # 1. Determine best answer tailored to question topic
    best_answer = ""
    topic_label = "System Architecture & Engineering"
    
    if any(k in q_lower for k in ["gil", "global interpreter lock"]):
        topic_label = "Python Concurrency & GIL"
        best_answer = (
            "The Global Interpreter Lock (GIL) is a mutual exclusion lock used by CPython to prevent "
            "multiple native threads from executing Python bytecodes simultaneously. For CPU-bound tasks, "
            "the GIL introduces contention, so multi-threading does not yield true parallel CPU speedups; "
            "instead, the multiprocessing module or C-extensions (like NumPy) must be used. For I/O-bound tasks, "
            "such as network requests or database access, asyncio or threading is highly effective because "
            "the GIL is released during system calls and socket wait operations, allowing other tasks to progress smoothly."
        )
    elif any(k in q_lower for k in ["asyncio", "async", "event loop", "coroutine"]):
        topic_label = "Asynchronous Python & Event Loops"
        best_answer = (
            "Python's asyncio is built upon single-threaded cooperative multitasking managed by an event loop. "
            "Coroutines declared with async def yield control back to the loop at await points. When designing high-throughput "
            "services in FastAPI, I/O-bound calls (e.g. database reads, external HTTP calls) must use async drivers (such as asyncpg or httpx) "
            "to prevent blocking the loop. For blocking legacy operations or CPU-intensive tasks, work should be offloaded to "
            "asyncio.to_thread or a ProcessPoolExecutor to ensure the server remains responsive to incoming requests."
        )
    elif any(k in q_lower for k in ["sql", "database", "index", "query", "optimization", "postgres"]):
        topic_label = "Database Design & Query Optimization"
        best_answer = (
            "When optimizing slow database operations, my first step is analyzing the execution plan using EXPLAIN ANALYZE "
            "to pinpoint sequential table scans, high buffer hit misses, or costly nested loop joins. I then create targeted "
            "B-Tree or composite indexes matching the filter and join predicates, while ensuring columns with high cardinality come first. "
            "On the application layer, I enforce strict connection pooling (e.g., PgBouncer), paginate large result sets with keyset cursor pagination, "
            "and eliminate N+1 query patterns using eager joins or batching."
        )
    elif any(k in q_lower for k in ["docker", "container", "kubernetes", "k8s", "deploy"]):
        topic_label = "Containerization & Cloud Infrastructure"
        best_answer = (
            "In production container environments, I implement multi-stage Dockerfiles to separate build dependencies from the final lightweight "
            "runtime container (e.g., utilizing Alpine or distroless base images), which minimizes the attack surface and image transfer latency. "
            "Containers are executed with non-root security contexts, explicit CPU and memory resource limits, and defined health and readiness probes. "
            "For orchestration in Kubernetes, deployment strategies use rolling updates with PodDisruptionBudgets to maintain high availability."
        )
    elif any(k in q_lower for k in ["react", "usememo", "usecallback", "hook", "render"]):
        topic_label = "React Performance & State Management"
        best_answer = (
            "In React, re-renders occur when state or props change. To optimize performance, useMemo caches the computed result of expensive calculations, "
            "while useCallback memoizes function instances across renders to preserve referential equality when passed to memoized children (React.memo). "
            "Beyond hooks, I identify unnecessary render cascades using the React DevTools Profiler, colocate state as close to consuming components as possible, "
            "and virtualize large data grids using libraries like react-window to maintain a 60 FPS user experience."
        )
    elif any(k in q_lower for k in ["api", "rest", "fastapi", "endpoint", "microservice"]):
        topic_label = "RESTful API Design & Resilience"
        best_answer = (
            "A resilient API architecture adheres strictly to REST principles with explicit HTTP verbs and standardized status codes. "
            "Key engineering requirements include rigorous input validation via schemas (such as Pydantic), idempotency keys for mutating endpoints, "
            "and distributed rate limiting (e.g., token bucket via Redis) to mitigate traffic spikes. In distributed microservices, "
            "I integrate circuit breakers, exponential backoff with jitter on outgoing calls, and correlation IDs in structured JSON logs for distributed tracing."
        )
    else:
        clean_q = question.replace('"', '').strip()
        best_answer = (
            f"An exemplary architectural answer for '{clean_q}': First, establish the core functional and non-functional requirements, "
            f"including throughput, latency, and consistency constraints. Second, outline the end-to-end technical implementation, "
            f"specifying appropriate data structures, design patterns, and separation of concerns. Third, discuss edge-case handling, "
            f"failure recovery mechanisms, and observability (metrics, structured logging, and alerting) necessary for enterprise production."
        )

    # 2. Analyze Candidate Answer & Build Personalized Feedback
    placeholders = {
        "",
    ...
    is_placeholder = clean_ans.lower() in placeholders or word_count < 4

    if is_placeholder:
        reasoning = f"No substantive technical content provided for {topic_label}. Expected concrete explanation."
        user_answer_comparison = (
            f"- **Observed Response:** No detailed technical explanation was captured for this question.\n"
            f"- **Expected Subject Depth:** The interviewer was evaluating core knowledge in **{topic_label}**.\n"
            f"- **Actionable Step:** When this question is asked in real interviews, structure your response using the STAR format: "
            f"define the concept, provide a concrete real-world engineering project example, and explain the trade-offs you navigated."
        )
        clarity = 2.0
        relevance = 2.0
        detail_level = 1.0
        ...
        # Extract keywords from the candidate's actual answer to quote and address
        ans_keywords = [w.strip(".,!?:;\"'()[]{}").capitalize() for w in words if len(w) > 4][:5]
        keywords_str = ", ".join(ans_keywords) if ans_keywords else "your initial points"
        
        clarity = min(9.5, max(4.0, 5.0 + (word_count // 15)))
        relevance = min(9.5, max(4.0, 6.0 + (word_count // 20)))
        detail_level = min(9.5, max(3.0, word_count / 12.0))
        factual_accuracy = 8.0
        confidence = min(9.0, max(4.0, 5.0 + (word_count // 25)))

        strong_point = f"You articulated clear thoughts regarding {keywords_str}."
        if word_count > 30:
            strong_point += f" Your explanation demonstrated good foundational familiarity with {topic_label}."
        else:
            strong_point += " You provided a direct and concise starting perspective."

        gap_points = (
            f"To elevate this answer from good to exceptional, dive deeper into production trade-offs: "
            f"discuss how you handle edge cases, measure operational latency, and prevent resource leaks under heavy traffic."
        )

        user_answer_comparison = (
            f"- **Strengths in your answer:** {strong_point}\n"
            f"- **Key areas to deepen:** {gap_points}\n"
            f"- **Comparison with benchmark:** Your answer ({word_count} words) touched on essential concepts, "
            f"while the benchmark answer integrates end-to-end architecture, specific design patterns, and operational observability."
        )
        reasoning = (
            f"Demonstrated solid conceptual awareness of {topic_label} ({word_count} words). "
            f"Clarity and technical depth can be further enhanced with concrete production examples."
        )
    ...
    return best_answer, user_answer_comparison, reasoning, metrics
...
def _evaluate_local(
    ...
    # Handle explicit silence
    if answer.strip() == "[No speech detected in audio]":
        return LLMEvaluationResult(
            score=0.0,
            reasoning="No speech detected in the audio recording.",
            best_answer="Please ensure your microphone is enabled and speak clearly into your mic.",
            user_answer_comparison="We could not detect audio speech from your microphone. Please verify your microphone permissions.",
            filler_word_count=0,
            metrics={"clarity": 0, "relevance": 0, "detail_level": 0, "factual_accuracy": 0, "confidence": 0},
    ...
    best_answer, user_answer_comparison, reasoning, metrics = _generate_personalized_best_answer_and_comparison(
        question=question,
        answer=answer,
    ...
    return LLMEvaluationResult(
        ...
        user_answer_comparison=user_answer_comparison,
...
class OpenAILLMProvider(LLMProvider):
    async def generate_technical_question(
        ...
        try:
            ...
            prompt = (
                f"Your name is James. You are a senior technical interviewer conducting a job interview for a {target_role} role.\n"
    ...
    async def evaluate_answer(
        ...
        if answer.strip() == "[No speech detected in audio]":
            return LLMEvaluationResult(
                score=0.0,
                reasoning="No speech detected in the audio recording.",
                best_answer="Please ensure your microphone is working and speak clearly.",
                user_answer_comparison="We couldn't hear any speech in your audio. Please check your mic.",
                filler_word_count=0,
                metrics={"clarity": 0, "relevance": 0, "detail_level": 0, "factual_accuracy": 0, "confidence": 0},
                factual_inaccuracies=[],
                weighted_breakdown={},
        ...
        try:
            client = _get_openai_client()
            prompt = (
                f"Your name is James. You are a senior technical interviewer conducting a job interview for a {agent_type} role.\n"
                ...
                "3. 'best_answer': Write a complete, highly-detailed, and professional AI-generated response that would score 100/100 for this exact question. Incorporate specific experiences, projects, and skills from the candidate's resume (if applicable) to make it a realistic and exemplary answer. Do NOT use generic advice. IMPORTANT: Vary your language significantly! Do NOT use repetitive or robotic openings like 'A good answer would...' or 'An ideal response...'. Just dive directly into the answer as if you were the candidate speaking natively.\n"
                "4. 'user_answer_comparison': Provide a structural and conceptual comparison between the candidate's answer and the best answer. Use markdown bullet points (- ) to clearly explain:\n"
                "   - What they missed structurally (e.g., missed the STAR method, lacked a concrete example).\n"
                "   - What technical concepts or resume experiences they failed to mention.\n"
                "   - How their approach differed from the ideal response.\n"
                "   IMPORTANT: Ensure extremely varied vocabulary and phrasing across evaluations. Avoid robotic, formulaic, or repetitive structures.\n"
                ...
                'Return JSON: {"metrics": {"clarity": 5, "relevance": 5, "detail_level": 5, "factual_accuracy": 5, "confidence": 5, "technical_depth": 5, "structure_and_flow": 5, "professionalism": 5}, "reasoning": "...", "best_answer": "...", "user_answer_comparison": "- Missed X\\n- Failed to mention Y", "filler_word_count": 0, "factual_inaccuracies": []}'
            )
            response = await client.chat.completions.create(
                model="gpt-4o-mini",
            ...
            fallback_best, fallback_comp, fallback_reason, _ = _generate_personalized_best_answer_and_comparison(
                question, answer, agent_type, resume_context
            )
            raw_best = (data.get("best_answer") or "").strip()
            raw_comp = (data.get("user_answer_comparison") or "").strip()
            
            # Guard against generic canned sentences even from external models
            if not raw_best or "A strong answer would directly address the question" in raw_best:
                raw_best = fallback_best
            if not raw_comp or "Compare your answer to the best answer" in raw_comp:
                raw_comp = fallback_comp

            return LLMEvaluationResult(
                score=final_score,
                reasoning=data.get("reasoning") or fallback_reason,
                best_answer=raw_best,
                user_answer_comparison=raw_comp,
                filler_word_count=filler_count,
                metrics=metrics,
            ...
            return _evaluate_local(question, answer, agent_type, resume_context)
    ...
    async def generate_learning_plan(
        ...
        try:
            client = _get_openai_client()
            prompt = (
                f"Your name is James. You are a senior technical interviewer and career coach for a {target_role} role.\n"
    ...
    async def parse_resume(self, text: str) -> dict:
        ...
        client = _get_openai_client()
        prompt = (
            "Your name is James. You are a professional resume parser for a technical interview platform.\n"
...
def get_llm_provider(use_fake: bool = False) -> LLMProvider:
    ...
    if not settings.openai_api_key.strip():
        if settings.environment in ("test", "development", "local"):
            logger.warning("OPENAI_API_KEY not configured — using FakeLLMProvider for local development")
            return FakeLLMProvider()
        raise RuntimeError(
